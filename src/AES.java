public class AES {
	 ...
	 */
	public static final int[] rcon = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

	private final int KEY_SIZE = 128;//the size of the key in bit
	private String key = "";
	static String iv = "";

	/**
	 * initialize AES and set the key
	 * @param key
	 * @throws Exception
	 */
	public AES(String key) throws Exception {
		...
		if(key.length() *4 != KEY_SIZE) //Check if the key size is 128 bits
		{
			throw new Exception("Error: Attemping to use a " + key.length() * 4 + "-bit key with AES-"+ KEY_SIZE);
		}
	 ...
	 */
	private int[][] subKey(int[][] keyMat, int index) {
		int[][] arr = new int[4][4];
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr.length; j++) {
				arr[i][j] = keyMat[i][4 * index + j];
			}
		}
		return arr;
	}
	...
	/**
	 * Inverse rendition of the subBytes. 
	 * The operations of invSubBytes are the reverse operations of subBytes.
	...
	{
		assert(arr.length == 4);//the condition in the assert must be true else its throw exception  
		if (times % 4 == 0) { //if the parameter is divide in 4 so its all the array
			return arr;
		}
		while (times > 0) { //else doing the shift  
			int temp = arr[0];
	...
	public int[][] keySchedule(String key)
	{
		...
		int bKeySize = key.length() * 4;
		int colToAdd = bKeySize + 48 - (32 * ((bKeySize / 64) - 2)); //size of key scheduling will be based on the binary size of the key.
		int[][] keyMatrix = new int[4][colToAdd / 4]; //creates the matrix for key scheduling
		int rconCount = 0;
		int[] last_col = new int[4];					
		final int keycounter = bKeySize / 32;
		int k;
		...
		while (keypoint < (colToAdd / 4)) {
			int temp = keypoint % keycounter;
			if (temp == 0) {
				for (k = 0; k < 4; k++) {
					last_col[k] = keyMatrix[k][keypoint - 1];
				}
				last_col = schedule_core(last_col, rconCount++);
				for (k = 0; k < 4; k++) {
					keyMatrix[k][keypoint] = last_col[k] ^ keyMatrix[k][keypoint - keycounter]; //W[i-1] ^ W[i-4]
				}
				keypoint++;
			}else {
				...
				while (keypoint < ktemp) {
					for (k = 0; k < 4; k++) {
						keyMatrix[k][keypoint] = keyMatrix[k][keypoint - 1] ^ keyMatrix[k][keypoint - keycounter]; //W[i] = W[i-1]^W[i-4]
					}
					keypoint++;
				}
			}
	...
	/**
	 * For every (binary key size / 32) the column in the expanded key. We compute a special column
	...
	public int[] schedule_core(int[] in, int rconpointer) {
		...
		in[0] ^= rcon[rconpointer]; 	//Xor with Rcon[j]
		return in;
	}
	 ...
	 */
	public void addRoundKey(int[][] bytematrix, int[][] keyMat)
	{
		for (int i = 0; i < bytematrix.length; i++) {
			for (int j = 0; j < bytematrix[0].length; j++) {
				bytematrix[j][i] ^= keyMat[j][i];
			}
		}
	}
	...
	{  int numRounds = 10 ;//+ (((key.length() * 4 - 128) / 32));
		...
		int[][] keyMat = this.keySchedule(key);
		if (!plainText.matches("[0-9A-F]+")) { //If line is valid (i.e. contains valid hex characters, encrpyt. Otherwise, skip line. 
			System.out.println("Not valid input!!! " +  plainText);
			System.exit(1);
		}
		stateMat = new int[4][4];
		for (int i = 0; i < 4; i++) //Parses line into a matrix
		{ for (int j = 0; j < 4; j++) 
				stateMat[j][i] = Integer.parseInt(plainText.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);}
		this.addRoundKey(stateMat, this.subKey(keyMat, 0)); //Starts the addRoundKey with the first part of Key Expansion
		for (int i = 1; i < numRounds; i++) {
			this.subBytes(stateMat); //implements the Sub-Bytes subroutine.
			this.shiftRows(stateMat); //implements Shift-Rows subroutine.
			this.mixColumns(stateMat);
			this.addRoundKey(stateMat, this.subKey(keyMat, i));
		}
		this.subBytes(stateMat); //implements the Sub-Bytes subroutine.
		this.shiftRows(stateMat); //implements Shift-Rows subroutine.
		this.addRoundKey(stateMat, this.subKey(keyMat, numRounds));
	    return MatrixToString(stateMat);
...
{   int numRounds = 10 ;//+ (((key.length() * 4 - 128) / 32));
	...
	int[][] keyMat = this.keySchedule(key);      
	stateMat = new int[4][4];
	for (int i = 0; i < stateMat.length; i++) //Parses line into a matrix
	{
		for (int j = 0; j < stateMat[0].length; j++) 
			stateMat[j][i] = Integer.parseInt(cipherText.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
	}
	this.addRoundKey(stateMat, this.subKey(keyMat, numRounds));
	for (int i = numRounds - 1; i > 0; i--) {
		this.invShiftRows(stateMat);
		this.invSubBytes(stateMat);
		this.addRoundKey(stateMat, this.subKey(keyMat, i));
		this.invMixColumns(stateMat);
	}
	this.invShiftRows(stateMat);
	this.invSubBytes(stateMat); 
	this.addRoundKey(stateMat, this.subKey(keyMat, 0));
	return MatrixToString(stateMat);}
}
